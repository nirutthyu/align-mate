"""
v2: New algorithm (18 questions) – exact backend logic
- Binary: Dice similarity
- Ordinal: 1 - |A-B|
- Complement: weighted mean absolute difference
- Risk penalty
...
# Trait weights for indices: stress(6), cleanliness(1), quiet_room(3), sharing_items(9)
TRAIT_WEIGHTS = {6: 1.5, 1: 1.3, 3: 1.2, 9: 1.1}
DEFAULT_WEIGHT = 1.0
...
def dice_similarity(a_bin, b_bin):
    inter = np.dot(a_bin, b_bin)
    total = np.sum(a_bin) + np.sum(b_bin)
    return 1.0 if total == 0 else (2 * inter) / total

def ordinal_similarity(a_ord, b_ord):
    return float(np.mean(1.0 - np.abs(a_ord - b_ord)))

def weighted_complement(A, B):
    total = 0.0
    wsum = 0.0
    for i in range(N_QUESTIONS):
        w = TRAIT_WEIGHTS.get(i, DEFAULT_WEIGHT)
        total += w * abs(A[i] - B[i])
        wsum += w
    return total / wsum
...
def compatibility(raw_a, raw_b):
    ...
    dice = dice_similarity(A[BINARY_IDX], B[BINARY_IDX])
    ord_sim = ordinal_similarity(A[ORDINAL_IDX], B[ORDINAL_IDX])
    similarity = (dice + ord_sim) / 2
    complement = weighted_complement(A, B)
    risk = conflict_risk(A, B)
    return ALPHA * similarity + BETA * complement - GAMMA * risk
